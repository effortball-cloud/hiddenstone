...
### 앱으로 설치하기 (PWA)
설치 없이 브라우저로 써도 되지만, **홈 화면에 추가하면 앱처럼 전체화면으로 실행**되고
**인터넷이 끊겨도 AI 대전·튜토리얼·기보 다시보기가 그대로 동작**합니다.
- 안드로이드 크롬: 메뉴 → 앱 설치 / 홈 화면에 추가
- 아이폰 사파리: 공유 → 홈 화면에 추가

스토어 출시 준비 상황과 남은 절차는 [LAUNCH.md](LAUNCH.md)에 정리해 두었습니다.
...
├─ vendor/         # PeerJS·MQTT 자체 호스팅 (CDN 의존 제거, 둘 다 MIT)
...
### 접근성
- 게임 모드·AI 난이도·맵·초읽기 카드는 모두 실제 `<button>`이라 Tab으로 이동하고
  Enter/Space로 선택됩니다(`aria-pressed`로 선택 상태를 알립니다).
- **바둑판도 키보드로 둘 수 있습니다** — 판을 Tab으로 포커스한 뒤 화살표로 커서를
  옮기고 Enter나 Space로 착수합니다. 커서 위치는 `aria-label`에 좌표와 그 자리의
  상태(빈 곳 / 흑돌 / 백돌)로 갱신되어 스크린리더가 읽습니다.
